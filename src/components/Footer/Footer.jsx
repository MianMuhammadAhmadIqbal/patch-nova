...
import { BsWhatsapp, BsInstagram, BsLinkedin, BsTwitterX, BsTelephone } from 'react-icons/bs'
      ...
      <div className="container-custom py-16 lg:py-20">
        <div className="grid lg:grid-cols-5 gap-12">
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center gap-2.5 mb-6">
              <div className="w-9 h-9 gradient-gold rounded-lg flex items-center justify-center">
                <span className="text-white font-display font-bold text-sm">BF</span>
              </div>
              <span className="font-display font-semibold text-lg text-cream tracking-tight">
                Brands<span className="text-gradient-gold">Face</span>
              </span>
            </div>
            <p className="text-white/50 text-sm leading-relaxed mb-6 max-w-xs">
              Packaging re-engineering and manufacturing firm helping brands transform their packaging into a powerful sales asset.
            </p>

            {/* Contact */}
            <div className="space-y-3">
              <a href="tel:+16038250565" className="flex items-center gap-3 text-sm text-white/50 hover:text-cream transition-colors group">
                <BsTelephone size={14} className="text-accent" />
                +1 603 825 0565
              </a>
              <a href="https://wa.me/16038250565" className="flex items-center gap-3 text-sm text-white/50 hover:text-cream transition-colors group">
                <BsWhatsapp size={14} className="text-green-500" />
                WhatsApp Us
              </a>
              <a href="mailto:hello@brandsface.com" className="flex items-center gap-3 text-sm text-white/50 hover:text-cream transition-colors group">
                <HiOutlineMail size={14} className="text-accent" />
                hello@brandsface.com
              </a>
            </div>
            ...
            <div className="flex items-center gap-3 mt-8">
              {[
                ...
                { Icon: BsWhatsapp, href: 'https://wa.me/16038250565', label: 'WhatsApp' },
              ].map(({ Icon, href, label }) => (
                <motion.a
                  key={label}
                  href={href}
          ...
          {Object.entries(footerLinks).map(([group, links]) => (
            <div key={group}>
              <div className="text-xs font-semibold tracking-[0.15em] uppercase text-accent mb-5">{group}</div>
      ...
      <div className="border-t border-white/[0.07]">
        <div className="container-custom py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-white/30">
            © {new Date().getFullYear()} BrandsFace. All rights reserved.
          </p>
